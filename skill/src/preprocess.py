...
官方限制（DeepSeek 视觉文档，本项目不自行放宽）：
- 单张图片 ≤ 32MiB（base64 / URL 方式）
- 单边 ≤ 8192px（同请求 ≥ 15 张时 4096px）
- 请求体 ≤ 48MiB（base64 计入）

下面这些不是官方规定，是本项目自己的工程策略：
- 请求体总量预算 44MiB：压在官方 48MiB 之下，避免 JSON 外壳和边界误差把请求顶过线
- 清晰度优先：先输出无损 PNG（截图/OCR/代码截图/表格/图表的文字最怕压缩失真），
  PNG 仍超限才改用 JPEG（质量 85）
- 仍然超限时按 0.85 逐步缩小尺寸，单张最多尝试 4 轮
- 多图合计超出预算时，优先压缩最大的那张
"""
